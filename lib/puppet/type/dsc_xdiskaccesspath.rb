...
Puppet::Type.newtype(:dsc_xdiskaccesspath) do
  ...
  validate do
      fail('dsc_accesspath is a required attribute') if self[:dsc_accesspath].nil?
    end
  ...
  # IsMandatory:  True
  # Values:       None
  newparam(:dsc_accesspath) do
    def mof_type; 'string' end
    def mof_is_embedded?; false end
    desc "AccessPath - Specifies the access path folder to the assign the disk volume to."
    isrequired
    validate do |value|
...
Puppet::Type.type(:dsc_xdiskaccesspath).provide :powershell, :parent => Puppet::Type.type(:base_dsc).provider(:powershell) do
