...
Puppet::Type.newtype(:dsc_sploglevel) do
  require Pathname.new(__FILE__).dirname + '../../' + 'puppet/type/base_dsc'
  require Pathname.new(__FILE__).dirname + '../../puppet_x/puppetlabs/dsc_type_helpers'

    class PuppetX::Dsc::TypeHelpers
      def self.validate_MSFT_SPLogLevelItem(mof_type_map, name, value)
        required = []
        allowed = ['area','name','tracelevel','eventlevel']
        ...
        lowkey_hash.keys.each do |key|
          if lowkey_hash[key]
            validate_mof_type(mof_type_map[key], 'MSFT_SPLogLevelItem', key, lowkey_hash[key])
          end
        end
      end
    end
  ...
  @doc = %q{
    The DSC SPLogLevel resource type.
    Automatically generated from
    'SharePointDsc/DSCResources/MSFT_SPLogLevel/MSFT_SPLogLevel.schema.mof'
  ...
  newparam(:dsc_name) do
    ...
    desc "Name - Friendly Name used to reference this collection of log level settings"
    isrequired
    validate do |value|
  ...
  # Name:         SPLogLevelSetting
  # Type:         MSFT_SPLogLevelItem[]
  # IsMandatory:  False
  # Values:       None
  newparam(:dsc_sploglevelsetting, :array_matching => :all) do
    ...
    validate do |value|
      ...
      (value.kind_of?(Hash) ? [value] : value).each_with_index do |v, i|
        fail "SPLogLevelSetting value at index #{i} should be a Hash" unless v.is_a? Hash

        PuppetX::Dsc::TypeHelpers.validate_MSFT_SPLogLevelItem(mof_type_map, "SPLogLevelSetting", v)
      end
    end
    munge do |value|
      value.kind_of?(Hash) ?
