...
Puppet::Type.newtype(:dsc_spsessionstateservice) do
  ...
  def dscmeta_resource_friendly_name; 'SPSessionStateService' end
  def dscmeta_resource_name; 'MSFT_SPSessionStateService' end
  ...
  newparam(:dsc_databasename) do
    ...
    desc "DatabaseName - The name of the database for the service"
    isrequired
    validate do |value|
  ...
  newparam(:dsc_databaseserver) do
    def mof_type; 'string' end
    def mof_is_embedded?; false end
    desc "DatabaseServer - The name of the database server for the database"
    isrequired
    validate do |value|
      unless value.kind_of?(String)
        fail("Invalid value '#{value}'. Should be a string")
  ...
  # IsMandatory:  False
  # Values:       None
  newparam(:dsc_sessiontimeout) do
