...
Puppet::Type.newtype(:dsc_spirmsettings) do
  ...
  # IsMandatory:  False
  # Values:       None
  newparam(:dsc_useadrms) do
    def mof_type; 'boolean' end
    def mof_is_embedded?; false end
    desc "UseADRMS - Use the RMS server published in this farm's Active Directory"
    validate do |value|
    end
  ...
  # IsMandatory:  False
  # Values:       None
  newparam(:dsc_rmsserver) do
    def mof_type; 'string' end
    def mof_is_embedded?; false end
    desc "RMSserver - Use the specified RMS server, must provide in URL format"
    validate do |value|
      unless value.kind_of?(String)
        fail("Invalid value '#{value}'. Should be a string")
      end
