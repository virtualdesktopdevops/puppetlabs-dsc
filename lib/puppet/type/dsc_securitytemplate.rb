...
Puppet::Type.newtype(:dsc_securitytemplate) do
  ...
  def dscmeta_resource_friendly_name; 'SecurityTemplate' end
  def dscmeta_resource_name; 'MSFT_SecurityTemplate' end
  ...
  newparam(:dsc_path) do
    def mof_type; 'string' end
    def mof_is_embedded?; false end
    desc "Path - The path to the desired security policy template (.inf)"
    validate do |value|
      unless value.kind_of?(String)
        fail("Invalid value '#{value}'. Should be a string")
      end
