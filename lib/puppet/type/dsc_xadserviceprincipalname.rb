...
Puppet::Type.newtype(:dsc_xadserviceprincipalname) do
  ...
  newparam(:dsc_ensure) do
    ...
    desc "Ensure - Specifies if the service principal name should be added or removed. Valid values are Present, Absent."
    validate do |value|
  ...
  newparam(:dsc_serviceprincipalname) do
    def mof_type; 'string' end
    def mof_is_embedded?; false end
    desc "ServicePrincipalName - Specifies the service principal name to add or remove from the account."
    isrequired
    validate do |value|
      unless value.kind_of?(String)
        fail("Invalid value '#{value}'. Should be a string")
  ...
  newparam(:dsc_account) do
    def mof_type; 'string' end
    def mof_is_embedded?; false end
    desc "Account - Specifies the user or computer account to change."
    validate do |value|
      unless value.kind_of?(String)
        fail("Invalid value '#{value}'. Should be a string")
      end
