...
Puppet::Type.newtype(:dsc_sqlaglistener) do
  ...
  @doc = %q{
    ...
    'SQLServerDsc/DSCResources/MSFT_SqlAGListener/MSFT_SqlAGListener.schema.mof'
  ...
  def dscmeta_resource_friendly_name; 'SqlAGListener' end
  def dscmeta_resource_name; 'MSFT_SqlAGListener' end
  ...
  newparam(:dsc_servername) do
    def mof_type; 'string' end
    def mof_is_embedded?; false end
    desc "ServerName - The host name or FQDN of the primary replica."
    validate do |value|
      unless value.kind_of?(String)
        fail("Invalid value '#{value}'. Should be a string")
      end
