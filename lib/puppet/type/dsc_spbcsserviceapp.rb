...
Puppet::Type.newtype(:dsc_spbcsserviceapp) do
  ...
  def dscmeta_resource_friendly_name; 'SPBCSServiceApp' end
  def dscmeta_resource_name; 'MSFT_SPBCSServiceApp' end
  ...
  newparam(:dsc_name) do
    ...
    desc "Name - The name of the BCS service app"
    isrequired
    validate do |value|
  ...
  newparam(:dsc_proxyname) do
    def mof_type; 'string' end
    def mof_is_embedded?; false end
    desc "ProxyName - The name of the BCS service application proxy"
    validate do |value|
      unless value.kind_of?(String)
        fail("Invalid value '#{value}'. Should be a string")
      end
  ...
  newparam(:dsc_applicationpool) do
    def mof_type; 'string' end
    def mof_is_embedded?; false end
    desc "ApplicationPool - The application pool it should run in"
    validate do |value|
      unless value.kind_of?(String)
        fail("Invalid value '#{value}'. Should be a string")
      end
  ...
  newparam(:dsc_databasename) do
    def mof_type; 'string' end
    def mof_is_embedded?; false end
    desc "DatabaseName - Name of the database to create for the service app"
    validate do |value|
      unless value.kind_of?(String)
        fail("Invalid value '#{value}'. Should be a string")
      end
  ...
  newparam(:dsc_databaseserver) do
    def mof_type; 'string' end
    def mof_is_embedded?; false end
    desc "DatabaseServer - Name of the database server to host the database on"
    validate do |value|
      unless value.kind_of?(String)
        fail("Invalid value '#{value}'. Should be a string")
      end
