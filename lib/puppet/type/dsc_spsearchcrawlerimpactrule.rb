...
Puppet::Type.newtype(:dsc_spsearchcrawlerimpactrule) do
  ...
  def dscmeta_resource_friendly_name; 'SPSearchCrawlerImpactRule' end
  def dscmeta_resource_name; 'MSFT_SPSearchCrawlerImpactRule' end
  ...
  newparam(:dsc_name) do
    def mof_type; 'string' end
    def mof_is_embedded?; false end
    desc "Name - The Site for the crawl impact rule"
    isrequired
    validate do |value|
      unless value.kind_of?(String)
        fail("Invalid value '#{value}'. Should be a string")
  ...
  # IsMandatory:  False
  # Values:       None
  newparam(:dsc_behavior) do
    def mof_type; 'string' end
    def mof_is_embedded?; false end
    desc "Behavior - The Behavior (RequestLimit or WaitTime) for this crawl impact rule"
    validate do |value|
      unless value.kind_of?(String)
        fail("Invalid value '#{value}'. Should be a string")
      end
  ...
  # IsMandatory:  False
  # Values:       None
  newparam(:dsc_requestlimit) do
    def mof_type; 'uint32' end
    def mof_is_embedded?; false end
    desc "RequestLimit - The RequestLimit setting for the crawl impact rule"
    validate do |value|
  ...
  newparam(:dsc_waittime) do
    def mof_type; 'uint32' end
    def mof_is_embedded?; false end
    desc "WaitTime - The WaitTime setting for the crawl impact rule"
    validate do |value|
      unless (value.kind_of?(Numeric) && value >= 0) || (value.to_i.to_s == value && value.to_i >= 0)
          fail("Invalid value #{value}. Should be a unsigned Integer")
      end
