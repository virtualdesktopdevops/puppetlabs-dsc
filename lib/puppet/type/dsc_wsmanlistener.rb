...
Puppet::Type.newtype(:dsc_wsmanlistener) do
  ...
  @doc = %q{
    ...
    'WSManDsc/Modules/WSManDsc/DSCResources/DSR_WSManListener/DSR_WSManListener.schema.mof'
  ...
  newparam(:dsc_issuer) do
    def mof_type; 'string' end
    def mof_is_embedded?; false end
    desc "Issuer - The Issuer of the certificate to use for the HTTPS WS-Man Listener."
    validate do |value|
      unless value.kind_of?(String)
        fail("Invalid value '#{value}'. Should be a string")
      end
  ...
  newparam(:dsc_subjectformat) do
    def mof_type; 'string' end
    def mof_is_embedded?; false end
    desc "SubjectFormat - The format used to match the certificate subject to use for an HTTPS WS-Man Listener. Valid values are Both, FQDNOnly, NameOnly."
    validate do |value|
      unless value.kind_of?(String)
        fail("Invalid value '#{value}'. Should be a string")
      end
  ...
  newparam(:dsc_matchalternate) do
    ...
    def mof_is_embedded?; false end
    desc "MatchAlternate - Should the FQDN/Name be used to also match the certificate alternate subject for an HTTPS WS-Man Listener."
    validate do |value|
    end
  ...
  newparam(:dsc_dn) do
    def mof_type; 'string' end
    def mof_is_embedded?; false end
    desc "DN - This is a Distinguished Name component that will be used to identify the certificate to use for the HTTPS WS-Man Listener."
    validate do |value|
      unless value.kind_of?(String)
        fail("Invalid value '#{value}'. Should be a string")
      end
  ...
  newparam(:dsc_hostname) do
    def mof_type; 'string' end
    def mof_is_embedded?; false end
    desc "HostName - The Host Name that an existing WS-Man Listener is bound to."
    validate do |value|
      unless value.kind_of?(String)
        fail("Invalid value '#{value}'. Should be a string")
      end
  ...
  newparam(:dsc_certificatethumbprint) do
    def mof_type; 'string' end
    def mof_is_embedded?; false end
    desc "CertificateThumbprint - The Certificate Thumbprint that an existing HTTPS WS-Man Listener is using."
    validate do |value|
      unless value.kind_of?(String)
        fail("Invalid value '#{value}'. Should be a string")
      end
