...
Puppet::Type.newtype(:dsc_sptrustedrootauthority) do
  ...
  newparam(:dsc_name) do
    ...
    desc "Name - Specifies the name of the trusted root authority to create."
    isrequired
    validate do |value|
  ...
  newparam(:dsc_certificatethumbprint) do
    def mof_type; 'string' end
    def mof_is_embedded?; false end
    desc "CertificateThumbprint - Specifies the X.509 certificate of the trusted root authority, as a certificate thumbprint."
    validate do |value|
      unless value.kind_of?(String)
        fail("Invalid value '#{value}'. Should be a string")
      end
