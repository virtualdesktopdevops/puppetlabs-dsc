...
Puppet::Type.newtype(:dsc_spmachinetranslationserviceapp) do
  ...
  def dscmeta_resource_friendly_name; 'SPMachineTranslationServiceApp' end
  def dscmeta_resource_name; 'MSFT_SPMachineTranslationServiceApp' end
  ...
  newparam(:dsc_name) do
    ...
    desc "Name - Specifies the name of the service application."
    isrequired
    validate do |value|
  ...
  newparam(:dsc_databasename) do
    def mof_type; 'string' end
    def mof_is_embedded?; false end
    desc "DatabaseName - Specifies the name of the database for the service application."
    validate do |value|
      unless value.kind_of?(String)
        fail("Invalid value '#{value}'. Should be a string")
      end
  ...
  newparam(:dsc_databaseserver) do
    def mof_type; 'string' end
    def mof_is_embedded?; false end
    desc "DatabaseServer - Specifies the name of the database server for the service application."
    validate do |value|
      unless value.kind_of?(String)
        fail("Invalid value '#{value}'. Should be a string")
      end
  ...
  newparam(:dsc_applicationpool) do
    def mof_type; 'string' end
    def mof_is_embedded?; false end
    desc "ApplicationPool - Specifies the application pool to use with the service application."
    validate do |value|
      unless value.kind_of?(String)
        fail("Invalid value '#{value}'. Should be a string")
      end
