...
Puppet::Type.newtype(:dsc_spsearchcrawlmapping) do
  ...
  def dscmeta_resource_friendly_name; 'SPSearchCrawlMapping' end
  def dscmeta_resource_name; 'MSFT_SPSearchCrawlMapping' end
  ...
  newparam(:dsc_url) do
    def mof_type; 'string' end
    def mof_is_embedded?; false end
    desc "Url - Source URI for the crawl mapping"
    isrequired
    validate do |value|
      unless value.kind_of?(String)
        fail("Invalid value '#{value}'. Should be a string")
  ...
  newparam(:dsc_target) do
    def mof_type; 'string' end
    def mof_is_embedded?; false end
    desc "Target - Target URI for the crawl mapping"
    validate do |value|
      unless value.kind_of?(String)
        fail("Invalid value '#{value}'. Should be a string")
      end
  ...
  newparam(:dsc_ensure) do
    def mof_type; 'string' end
    def mof_is_embedded?; false end
    desc "Ensure - Ensure the crawl mapping is Present or Absent Valid values are Present, Absent."
    validate do |value|
